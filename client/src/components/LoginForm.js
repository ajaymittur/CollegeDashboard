			...
			<Grid.Column style={{ maxWidth: 450 }}>
				...
				<Form error size='large' onSubmit={handleSubmit}>
					<Segment raised inverted color='teal' secondary className='zoomIn'>
						<Form.Input fluid onChange={handleChange} icon='address card' iconPosition='left' placeholder='Email' name='email' type='input' className='zoomIn' />
						<Form.Input fluid onChange={handleChange} icon='lock' iconPosition='left' placeholder='Password' name='password' type='password' className='zoomIn' />
						<Button type='submit' className='zoomIn' loading={load} onClick={() => setLoad(true)}>
							Login
						</Button>
