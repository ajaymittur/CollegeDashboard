					...
					<Grid.Column style={{ maxWidth: 500 }}>
						...
						<Form error size='large' onSubmit={handleSubmit}>
							<Segment raised inverted color='teal' secondary size='large' textAlign='left' className='zoomIn'>
								...
								<Form.Group widths='equal'>
									<Form.Input fluid required onChange={handleChange} label='Enter Email' placeholder='Email' name='email' type='input' className='zoomIn' value={formData.email || ""} />
									<Form.Input fluid required onChange={handleChange} label='Enter Name' placeholder='Name' name='name' type='input' className='zoomIn' value={formData.name || ""} />
									<Form.Input fluid required onChange={handleChange} label='Enter USN' placeholder='USN' name='usn' type='input' className='zoomIn' value={formData.usn || ""} />
								</Form.Group>

								<Form.Input fluid required onChange={handleChange} label='Enter Password' placeholder='Password' name='password' type='password' className='zoomIn' value={formData.password || ""} />
								<Form.Input
									fluid
									required
									onChange={handleChange}
								...
								/>
								<Form.Input required label='Number of subjects' name='numsubjects' onChange={handleChangeNumSubjects} type='input' className='zoomIn' value={formData.numsubjects || ""} />
								<Button
									type='button'
									onClick={() => {
										if (!formData.profilepic || !formData.profilepic.length)
							...
							<Segment raised inverted color='teal' secondary size='large' textAlign='left' className='zoomIn'>
								{[...Array(subCount)].map((e, i) => (
									<Form.Group widths='equal' key={i}>
										<Form.Input fluid required name={`subject${i + 1}`} label={`Subject ${i + 1}`} onChange={handleChange} className='zoomIn' value={formData[`subject${i + 1}`] || ""} />
										<Form.Input fluid required name={`credits${i + 1}`} label='Credits' onChange={handleChange} className='zoomIn' value={formData[`credits${i + 1}`] || ""} />
										<Form.Input fluid required name={`marks${i + 1}`} label={`Marks`} onChange={handleChange} className='zoomIn' value={formData[`marks${i + 1}`] || ""} />
										<Form.Input fluid required name={`attd${i + 1}`} label='Attendance' onChange={handleChange} className='zoomIn' value={formData[`attd${i + 1}`] || ""} />
									</Form.Group>
								))}
